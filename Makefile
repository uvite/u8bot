...
FRONTEND_EXPORT_DIRs = /uvite/u8ui/dist
...
pkg/server/assets.go:
	go run ./utils/embed -package server -tag web -output $@ $(FRONTEND_EXPORT_DIRs)

staticm:   pkg/server/assets.go pkg/backtest/assets.go
