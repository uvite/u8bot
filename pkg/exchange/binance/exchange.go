...
func (e *Exchange) setServerTimeOffset(ctx context.Context) {
	...
	if err != nil {
		log.WithError(err).Error("can not set server time 333")
	}

	_, err = e.futuresClient.NewSetServerTimeService().Do(ctx)
	if err != nil {
		log.WithError(err).Error("can not set server time 2222")
	}

	if err = e.client2.SetTimeOffsetFromServer(ctx); err != nil {
		log.WithError(err).Error("can not set server time 111")
	}
}
