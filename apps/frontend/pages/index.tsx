...
import {queryAssets, queryPnl, querySessions, queryStrategies} from '../api/bbgo';
import {getCookie,setCookie} from '../api/cooke';

import { ChainId, Config, DAppProvider } from '@usedapp/core';
import { Theme } from '@mui/material/styles';
import {useEffect} from "_@types_react@18.0.24@@types/react";
...
export default function Home() {
  ...
  React.useEffect(() => {
    queryStrategies(  (data) => {

      setCookie("exchange",data[0].on[0])
      setCookie("symbol",data[0][data[0].strategy].symbol)
      setCookie("Status",data[0][data[0].strategy].Status)
      setCookie("strategy",data[0].strategy)
      console.log(data )
    });
